...
// Generates next 6 calendar days dynamically for a premium chip selector
...
export default function ScheduleViewingScreen({ route, navigation }: any) {
  ...
  const [calendarDays, setCalendarDays] = useState<ReturnType<typeof generateNextDays>>([]);

  // Extract property details passed from detail screen
  const propertyId = route.params?.propertyId || "2";
  const propertyTitle = route.params?.propertyTitle || "Modern Villa with Garden";
  const propertyPrice = route.params?.propertyPrice || "₹2.5 Cr";
  const propertyLocation = route.params?.propertyLocation || "OMR, Chennai";
